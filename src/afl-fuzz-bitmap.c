...
inline u8 has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  u8  ret = 0;
  u64 new_bytes = 0;
  u8  bounus = 1;  // actually 8
  while (i--) {
    if (unlikely(*current)) discover_word(&ret, current, virgin);

    current++;
    virgin++;
    if (ret == 2) { new_bytes += bounus; }
    if (i < 64) {  // TIS bitmap area, increase the bounus
      bounus = 2;  // actually 16
    }
  }
  ...
  // see if interesting by new bytes number
  u64 if_interesting_possibility = 0;

  if (new_bytes != 0) {
    if (new_bytes > max_new_byte) max_new_byte = new_bytes;
    if (new_bytes < min_new_bit) min_new_bit = new_bytes;

    if_interesting_possibility =
        (max_new_byte - min_new_bit) == 0
            ? 1.0
            : (new_bytes - min_new_bit) / (max_new_byte - min_new_bit);
  }
  FILE *f = fopen("interesting_possibility.txt", "a");
  fprintf(f, "if_interesting_possibility: %f\n", if_interesting_possibility);
  fprintf(f, "new_bytes: %f\n", new_bytes);
  fprintf(f, "min_new_bit: %f\n", min_new_bit);
  fprintf(f, "max_new_byte: %f\n", max_new_byte);
  fclose(f);

  // if(return_zero_with_probability(1.0-if_interesting_possibility) == 0){
  //   return 0;
  // }
...
int return_zero_with_probability(double p) {
  if (p < 0.0 || p > 1.0) {
    fprintf(stderr, "Probability value 'p' must be between 0 and 1.\n");
    return -1;  // Return an error or handle as appropriate
  }
  srand(time(NULL));  // Note: ideally, srand should only be called once
  double randProb =
      rand() / (RAND_MAX + 1.0);  // Normalize the random number to [0,1]
  if (randProb < p) {
    return 0;
  } else {
    return 1;
